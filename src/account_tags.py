'''
Rule Name:
  ACCOUNT_TAGS_REQUIRED

Description:
  Ensure that AWS Accounts has the required tags.

Rule Parameters:
  | ------------------- | --------- | ----------------------------------------------- |
  | Parameter Name      | Type      | Description                                     |
  | ------------------- | --------- | ----------------------------------------------- |
  | RequiredTagsList    | Mandatory | List of required tags, seperated by commas.     |
  | ------------------- | --------- | ----------------------------------------------- |
  | RoleMasterAssume    | Optional  | ARN of the rol in master account to assume      |
  | ------------------- | --------- | ----------------------------------------------- |
...
Feature:
  ...
  I want: To ensure all the AWS Accounts have the required tags.
...
Scenarios:
  ...
  Scenario: 2
    Given: RequiredTagsList is not empty
      And: A user listed in RequiredTagsList is not an alphanumerical string
     Then: Return an error

  Scenario: 3
    Given: RequiredTagsList is not empty
      And: RequiredTagsList is configured correctly
      And: At least one tags required is not present in the account.
     Then: Return Non-Compliant
'''
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    organization_client = get_client('organizations', valid_rule_parameters['role_arn'],True)
    account_id = event['accountId']
    account_tags = get_all_tags(organization_client, account_id)
    account_tags_keys = []
    account_tags_values = []
    for tag in account_tags:
        account_tags_keys.append(tag['Key'])
        account_tags_values.append(tag['Value'])

    for req in valid_rule_parameters['tags']:
        if req not in account_tags_keys:
            print('The account ({}) does not have the mandatory tag ({}).'.format(account_id, req))
            return build_evaluation(account_id, 'NON_COMPLIANT', event, annotation='The account ({}) does not have the mandatory tag ({}).'.format(account_id, req))    
    return 'COMPLIANT'
...
def evaluate_parameters(rule_parameters):
    ...
    if "RequiredTagsList" in rule_parameters:
        valid_rule_parameters['tags'] = []
        tags = rule_parameters['RequiredTagsList'].split(',')
        for tag in tags:
            tag = tag.strip()
            valid_rule_parameters['tags'].append(tag)
    if "RoleMasterAssume" in rule_parameters:
        valid_rule_parameters['role_arn'] = rule_parameters['RoleMasterAssume']
...
def build_evaluation(resource_id, compliance_type, event, resource_type=DEFAULT_RESOURCE_TYPE, annotation=None):
    ...
    event -- the event variable given in the lambda handler
    resource_type -- the     (or AWS::::Account) to report on the rule (default DEFAULT_RESOURCE_TYPE)
...
def lambda_handler(event, context):
    ...
    try:
        AWS_CONFIG_CLIENT = get_client('config', event, False)
